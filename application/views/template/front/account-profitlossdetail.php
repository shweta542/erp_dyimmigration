<?php $arrpro = explode('|',$privileges_settings->ProfitLoss) ?>
<?php 
if (isset($_GET['branch'])) {
    ...
    $subheaddata1=$this->db->select('h.heads_id as heads_id,h.status as headstatus,h.heads_name as heads_name,j.*')->
            ...
            ->where('h.status',2)
             ->where('j.date >=',$fdate)
            ->where('j.date <=',$tdate)
            ->where('j.moneyInOut',$inout)
            ->where('j.sub_head_id',$subId)
	...
	<div class="content container-fluid">
		<div class="page-header">
			<div class="row align-items-center">
				<div class="col">
					<h3 class="page-title">Transaction Summary</h3>
					<ul class="breadcrumb">
		 ...
		 <form action="profitlossdetail.html/<?= $subId ?>/<?= $inout ?>" method="get">
